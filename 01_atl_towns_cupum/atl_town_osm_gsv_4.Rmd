...
```{r}
edges_long_than_100_split <- st_split(edges_long_than_100, nodes) %>%  # st_split
...
g3 <- edges_long_than_100_split %>%
  st_drop_geometry() %>%
  filter(length_segmented > 25 & length_segmented < 100) %>%
  ggplot(aes(x = length_segmented, fill = road_type)) + 
  geom_histogram()
...
```{r}
set.seed(123)
edges_segmented_sample <- edges_segmented %>%
  group_by(road_type) %>%
  slice_sample(n = 200) %>%
  ungroup()
edges_segmented_sample
```

```{r}
nodes_trimmed <- nodes %>%
  filter(lengths(st_intersects(., edges_segmented_sample)) > 0)
```
...
tm_shape(edges_segmented_sample) +
...
g1 <- edges %>%
  ...
  ggplot(aes(x = length)) + 
  geom_histogram()
g2 <- edges_segmented_trimmed_sample %>%
  st_drop_geometry() %>%
  ggplot(aes(x = length, fill = highway)) + 
  geom_histogram()
ggpubr::ggarrange(g1, g2)
```

```{r}
edges_segmented_trimmed_sample <- edges_segmented_trimmed_sample %>%
  rename(edge_id = roadseg_id) %>%
  mutate(edge_id = as.character(seq(1, nrow(.)))) %>%
  rename(road_type = highway) %>%
  mutate(length = as.numeric(length)) %>%
  relocate(edge_id, osm_id, road_type)
edges_segmented_trimmed_sample
```

```{r}
edges_segmented_trimmed_sample %>%
  st_write("./assets/osm/atl_towns_edges_300.geojson")
```
